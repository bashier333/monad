...
const EXPECTED: Record<string, string[]> = {
  ...
  "/ontology/links": ["POST", "GET", "DELETE"],
  ...
  "/agent/confirm": ["POST"],
  "/agent/evals": ["GET"],
  "/settings/provider-key": ["GET", "POST", "DELETE"],
  "/sync/runs": ["GET"],
  "/sync/pull": ["POST"],
  "/sync/pipelines": ["POST", "GET"],
  "/mcp": ["GET"],
  "/boards": ["POST", "GET"],
  "/boards/[id]": ["GET", "PUT", "DELETE"],
  "/boards/[id]/revert": ["POST"],
  "/boards/[id]/publish": ["POST"],
  "/boards/[id]/favorite": ["POST"],
  "/automations": ["POST", "GET"],
  "/automations/[key]": ["GET", "PATCH", "POST"],
  "/boards/shared/[token]": ["GET"],
  "/desktop": ["GET"],
...
describe("API contract surface (B-084)", () => {
  ...
  it("every route enforces auth or is explicitly public", () => {
    const publicRoutes = new Map([
      ...
      ["/boards/shared/[token]", "revocable share-token link, published boards only, no owner identity"],
