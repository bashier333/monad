# Connector: Paginated REST (`rest-paginated`)

- Kind: rest
- Auth: endpoint or REST_PAGINATED_URL (server secret store only; zero values in repo)
...
Polls `?cursor=&from=&to=`. Upstream returns `{rows:[{_key,_date,...}], nextCursor}`. 429 pauses the schedule 60s.
