# Connector: SQLite export bridge (`sqlite-file`)

- Kind: rest
- Auth: endpoint or SQLITE_BRIDGE_URL (server secret store only; zero values in repo)
...
BRIDGE CONTRACT: operator exposes a table export as `{rows,nextCursor}` JSON. Full-set pulls tombstone vanished keys.
