...
5. **Per-pack (E-474):** compare `PACK_PRICES` (freight + agency Team lines on `/pricing`)
   against the Stripe price metadata per pack. Same price today; any split must land
   in code first, then Stripe.
