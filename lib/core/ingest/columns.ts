...
export interface ColumnSuggestion<F extends string = CanonicalField> {
  mapping: Partial<Record<F, number>>;
  confidence: Partial<Record<F, number>>;
  unmapped: number[];
}

export function detectColumns(headers: string[]): ColumnSuggestion<CanonicalField>;
...
export function detectColumns<F extends string>(
  headers: string[],
  fields: readonly F[] = CANONICAL_FIELDS as unknown as readonly F[],
  aliases: Record<F, string[]> = ALIASES as unknown as Record<F, string[]>,
): ColumnSuggestion<F> {
  const mapping: Partial<Record<F, number>> = {};
  const confidence: Partial<Record<F, number>> = {};
  const claimedIdx = new Set<number>();
  const claimedField = new Set<F>();
  ...
  for (const pass of passes) {
    keys.forEach((h, idx) => {
      ...
      let best: { field: F; conf: number } | null = null;
...
export function applyMapping<F extends string>(
  ...
  mapping: Partial<Record<F, number>>,
  fields: readonly F[],
): Array<{ rowNumber: number; record: Record<F, string> }>;
export function applyMapping<F extends string>(
  headers: string[],
  rows: string[][],
  mapping: Partial<Record<F, number>>,
