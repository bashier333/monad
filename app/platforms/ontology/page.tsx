...
function ObjectBox({ x, y }: { x: number; y: number }) {
  ...
  return (
    <g>
      <rect x={x} y={y} width={180} height={124} rx={5} fill="#fafaf9" stroke={STROKE} strokeWidth={1} />
      ...
      {rows.map((r, i) => {
        ...
        return (
          <g key={r.t}>
            {i > 0 && <line x1={x} y1={y + 34 + 22.5 * i} x2={x + 180} y2={y + 34 + 22.5 * i} stroke="#d6d3d1" strokeWidth={1} />}
...
function LinkPill({ x, y }: { x: number; y: number }) {
  return (
    <g>
      <rect x={x - 27} y={y - 12} width={54} height={24} rx={12} fill="#fafaf9" stroke={STROKE} strokeWidth={1} />
...
function ObjectDiagram() {
  ...
  return (
    <svg viewBox="0 0 1200 760" className="w-full" role="img" aria-label="Diagram of linked objects around one ontology">
      <rect x={60} y={24} width={1080} height={712} rx={10} fill="#fafaf9" stroke={STROKE} strokeWidth={1} />
      ...
      <rect x={505} y={296} width={190} height={168} rx={5} fill="#fafaf9" stroke={STROKE} strokeWidth={1} />
    ...
    <main className={`${sans.className} bg-stone-50 text-stone-900`}>
      ...
      <header className="border-b border-stone-200">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 md:px-8">
          ...
          <nav className="hidden items-center gap-7 text-[15px] text-stone-600 md:flex" aria-label="Page">
            <Link href="/platforms/ontology" className="font-semibold text-stone-900">
              Ontology
            </Link>
      ...
      <section className="overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 pb-16 pt-16 text-center md:px-8 md:pt-24">
          <Reveal>
            ...
            <p className="mx-auto mt-6 max-w-[58ch] text-[17px] leading-relaxed text-stone-600">
            ...
            <div className="mt-8">
              <a
                ...
                download
                className="cta-lift inline-block whitespace-nowrap rounded-full bg-emerald-700 px-7 py-3 text-[16px] font-bold text-white hover:bg-emerald-800"
              >
                Download for Windows
              </a>
              <p className="mx-auto mt-3 max-w-[40ch] text-xs leading-relaxed text-stone-500">
      ...
      <section className="border-t border-stone-200">
        <div className="mx-auto max-w-7xl px-4 py-16 text-center md:px-8 md:py-24">
          <Reveal>
            <p className="text-[15px] font-semibold tracking-wide text-stone-500">Ontology Engine</p>
          ...
          <div className="mx-auto mt-12 grid max-w-5xl gap-4 md:grid-cols-3">
            {DOCS.map((d, i) => (
              <Reveal key={d.n} delay={i * 80}>
                <div className="h-full rounded-2xl border border-stone-200 bg-white p-6 text-left">
                  <p className={`${mono.className} text-xs text-stone-500`}>{d.n}</p>
                  <div className="mt-4 h-2 w-3/4 rounded-full bg-stone-200" />
                  <div className="mt-2 h-2 w-1/2 rounded-full bg-stone-200" />
                  <p className="mt-4 text-[13px] text-stone-400">Placeholder</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="border-t border-stone-200">
        <div className="mx-auto max-w-5xl px-4 py-16 md:px-8 md:py-24">
          ...
          <Reveal delay={120}>
            <p className="mx-auto mt-10 max-w-[58ch] text-center text-[16px] leading-relaxed text-stone-600">
