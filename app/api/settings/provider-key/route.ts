...
async function ownerOrgId() {
  ...
  return { orgId: active.organization.id, userId };
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    await logAccess(res.orgId, res.userId, "settings:provider-key-save", body.provider);
...
export async function DELETE(req: Request) {
  ...
  await logAccess(res.orgId, res.userId, "settings:provider-key-remove", body.provider);
