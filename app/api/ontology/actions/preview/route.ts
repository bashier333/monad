...
import { dryRunSet, type ActionEffect } from "@/lib/core/ontology/actions";
...
export async function POST(req: Request) {
  ...
  const body = (await req.json()) as { actionKey?: string; objectId?: string; inputs?: unknown };
  ...
  for (const e of action.effects as unknown[] as ActionEffect[]) {
    const r = resolveEffect(e, inputs);
    if (!r.value) continue;
    changes.push(...dryRunSet((object.data as Record<string, unknown>) ?? {}, [r.value]));
  }
  return NextResponse.json({
    preview: { changes, approvalRequired: action.approvalPolicy !== "none", approvalPolicy: action.approvalPolicy },
  });
}
